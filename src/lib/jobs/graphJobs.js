  ...
  for (const substitution of substitutions) {
    try {
      ...
      if (currentOwner || currentMember) {
        ...
        logger.info(`${logPrefix} - The substitute {SubstituteId} is not an owner or a member of the team {SubstitutionTeamId} - can set substitution status to 'expired'`, substitution.substituteId, substitution.teamId)
      }
      ...
      logger.errorException(error, `${logPrefix} - An error occured while trying to deactivate the substitutionId {SubstitutionId} for team {SubstitutionTeamId}`, substitution.substituteId, substitution.teamId)
