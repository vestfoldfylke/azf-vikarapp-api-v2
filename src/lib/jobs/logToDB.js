...
const logToDB = async (type = 'info', data, request, context, requestor) => {
  ...
  try {
    ...
    // Remove request headers and rawBody
    const apiRequest = { ...request }
    delete apiRequest.headers
    delete apiRequest.rawBody
    const method = request?.method || 'unknown'
    const origin = request?.headers?.origin || request?.headers?.referer || context?.request?.headers?.origin || context?.request?.headers?.referer || 'unknown'
